...
//    매월 1일 마다 : 0 0 0 1 * ?
    ...
    @Autowired
    private PloAttachFileDAO ploAttachFileDAO;
    ...
    public void checkFiles() {
        ...
        //어제 첨부파일 목록
        List<PloAttachFileVO> fileVOList = ploAttachFileDAO.getOldFiles();

        //원본 경로
        List<Path> fileListPaths = fileVOList.stream().map(attach ->
                Paths.get("C:/upload/plogging", attach.getUploadPath(), attach.getUuid() + "_" + attach.getFileName())
        ).collect(Collectors.toList());

        //썸네일 경로를 원본 경로 List에 추가
        fileVOList.stream().filter(attach -> attach.isImage()).map(attach ->
                Paths.get("C:/upload/plogging", attach.getUploadPath(), "s_" + attach.getUuid() + "_" + attach.getFileName()))
                .forEach(path -> fileListPaths.add(path));

        //어제 업로드 된 폴더의 경로
        File dir = Paths.get("C:/upload/plogging", getFolderYesterday()).toFile();
