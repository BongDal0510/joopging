...
//    매월 1일 마다 : 0 0 0 1 * ?
    ...
    public void checkFiles() {
        ...
        //어제 첨부파일 목록
        List<CommuAttachFileVO> fileVOList = commuattachFileDAO.commuGetOldFiles();

        //원본 경로
        List<Path> fileListPaths = fileVOList.stream().map(attach ->
                Paths.get("C:/upload/commu", attach.getCommuUploadPath(), attach.getCommuUuid() + "_" + attach.getCommuFileName())
        ).collect(Collectors.toList());

        //썸네일 경로를 원본 경로 List에 추가
        fileVOList.stream().filter(attach -> attach.isCommuImage()).map(attach ->
                Paths.get("C:/upload/commu", attach.getCommuUploadPath(), "s_" + attach.getCommuUuid() + "_" + attach.getCommuFileName()))
